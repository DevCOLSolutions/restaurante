        ...
        <div className="mt-4 space-y-2">
          {filtered.map((mesero) => (
            <div key={mesero.id} className="rounded-2xl bg-white border border-neutral-200 p-4 flex items-center justify-between shadow-sm">
              ...
              <select
                value={mesero.status}
                onChange={(e) => setMeseros((prev) => prev.map((m) => m.id === mesero.id ? { ...m, status: e.target.value } : m))}
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-sm text-neutral-400">No hay meseros en esta categoría</div>
          )}
        </div>
      </div>

      {/* Modal administrar meseros */}
      {showManage && (
              ...
              <div className="space-y-2 mb-3">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-neutral-200 bg-white pl-8 pr-3 py-2 text-xs outline-none focus:border-neutral-400"
                  />
                </div>
                ...
                <p className="text-[10px] text-neutral-400 -mt-1">Estado en que se encuentra el mesero</p>
              </div>
              <button
                onClick={() => {
                  if (!newName.trim() || !newUsername.trim() || !newEmail.trim() || !newPassword.trim()) return
                  const nuevo: Mesero = {
                    id: `e${Date.now()}`,
                    name: newName.trim(),
              ...
              {meseros.map((mesero) => (
                <div key={mesero.id} className="rounded-2xl bg-white border border-neutral-200 p-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <input
                      value={mesero.name}
                      onChange={(e) => setMeseros((prev) => prev.map((m) => m.id === mesero.id ? { ...m, name: e.target.value } : m))}
                      className="w-full rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-sm font-semibold text-neutral-900 outline-none focus:border-neutral-400"
                    />
                    <div className="flex gap-1.5">
                      <input
                        value={mesero.username}
                        onChange={(e) => setMeseros((prev) => prev.map((m) => m.id === mesero.id ? { ...m, username: e.target.value } : m))}
                        className="flex-1 rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-xs text-neutral-600 outline-none focus:border-neutral-400"
                        placeholder="Usuario"
                      />
                      <input
                        value={mesero.email}
                        onChange={(e) => setMeseros((prev) => prev.map((m) => m.id === mesero.id ? { ...m, email: e.target.value } : m))}
                        className="flex-1 rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-xs text-neutral-600 outline-none focus:border-neutral-400"
                        placeholder="Email"
                      />
                    </div>
                    <div className="flex gap-1.5">
                      {changingPass === mesero.id ? (
                        <div className="flex-1 flex gap-1">
                          <input
                            value={changingPassValue}
                            onChange={(e) => setChangingPassValue(e.target.value)}
                            className="flex-1 rounded-lg border border-neutral-200 bg-white px-2 py-1 text-xs outline-none focus:border-neutral-400"
                            placeholder="Nueva contraseña"
                          />
                          <button
                            onClick={() => {
                              if (changingPassValue.trim()) {
                                setMeseros((prev) => prev.map((m) => m.id === mesero.id ? { ...m, password: changingPassValue.trim() } : m))
                              }
                              setChangingPass(null)
                              setChangingPassValue("")
                            }}
                            className="rounded-lg bg-neutral-900 text-white px-2.5 py-1 text-[10px] font-medium hover:bg-neutral-800 transition-colors"
                          >
                            OK
                          </button>
                        </div>
                        ...
                        <button
                          onClick={() => { setChangingPass(mesero.id); setChangingPassValue("") }}
                          className="flex-1 flex items-center justify-center gap-1 rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-[10px] text-neutral-500 hover:bg-neutral-100 hover:text-neutral-700 transition-colors"
                        >
                          <KeyRound size={12} />
                          Cambiar contraseña
                        </button>
                  ...
                  <button
                    onClick={() => setMeseros((prev) => prev.filter((m) => m.id !== mesero.id))}
